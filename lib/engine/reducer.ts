...
import type { EngineInternals, EngineState, ProposedEffect, SystemHandlers } from './types';

/**
 * THE PURE CORE.
 *
 * `step` is synchronous, total, and free of clocks and randomness. Given the same
...
export function step(state: EngineState, event: CanonicalEvent, deps: StepDeps): StepResult {
  ...
  const verifications: VerificationRecord[] = [...outcome.verifications];

  for (const proposed of outcome.effects) {
    ...
    const effect: SideEffect = {
      ...
      // Nothing in this build leaves the process. This is asserted in tests.
      ...
      verifications.push({
        ...
        detail:
          status === 'EXECUTED'
            ...
            : `Not evaluated: side effect resolved as ${status}.`,
      });
    }
  }
  ...
  if (outcome.transitionTo !== undefined && outcome.transitionTo !== state.lifecycleState) {
    const rule = system.lifecycle.transitions.find(
      (t) => t.from === state.lifecycleState && t.to === outcome.transitionTo,
    );

    if (rule === undefined) {
      transitions.push({
        id: `${event.eventId}:transition`,
        eventId: event.eventId,
        from: state.lifecycleState,
        to: outcome.transitionTo,
        mechanism: 'DETERMINISTIC_RULE',
        accepted: false,
        rejectionReason: `No declared transition permits ${state.lifecycleState} -> ${outcome.transitionTo}. State did not move.`,
      });
    } else {
      transitions.push({
        id: `${event.eventId}:transition`,
      ...
      nextLifecycleState = outcome.transitionTo;
    }
  }
...
function resolveEffect(
  ...
  if (!proposed.policyPermits) {
    return {
      ...
      detail: proposed.policyReason ?? 'Blocked by policy.',
    };
  }
