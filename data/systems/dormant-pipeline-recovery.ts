...
const RAW = {
  ...
  failureModes: [
    ...
    },
    {
      ...
      class: 'RATE_LIMITED',
      failure: 'The outreach provider refuses further sends partway through a cycle.',
      cause: 'A cycle evaluates a large segment and despatches faster than the provider\u2019s published limit allows.',
      businessImpact: 'Part of a campaign silently fails to send while the run records the whole batch as processed, so the shortfall is invisible.',
      ...
      detection: 'A rate-limit response from the provider, or a send-rate breach detected before the call is made.',
