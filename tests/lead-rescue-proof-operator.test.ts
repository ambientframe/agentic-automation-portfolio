...
/**
 * The operator boundary has two request contracts in flight, and it reports the SAME event in
 * two different shapes depending on which one is in force. Under the older contract an authority
 * refusal is an outcome token; under the authenticated one it is a top-level error carrying the
 * principal the runtime resolved.
 *
 * These fixtures are transcripts of real responses, taken from a running server on each contract
 * rather than written from the type definitions — which is the only version of this test worth
 * having, since the failure being guarded against is precisely a wrong belief about the wire.
 *
 ...
 */
describe('an authority refusal reads as the gate working, on either contract', () => {
  it('reports the outcome-token form as a refusal, not an error', () => {
  ...
  it('reports the authenticated form as the same refusal, and keeps the reason it gave', () => {
